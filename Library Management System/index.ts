...
class Book {
  ...
  checkBook(): void {
    if (!this.isAvailable) {
      console.log(
        `${this.title} is currently not available, kindly check back`
      );
      return;
    }
...
class Member {
  ...
  returnBook(book: Book): void {
    ...
    if (bookIndex === -1) {
      console.log(
        `${book.title} is not found in borrowed books of ${this.name}`
      );
      return;
    }
...
class Library {
  ...
  removeBook(book: Book): void {
    ...
    if (!book.isAvailable) {
      console.log(
        `${book.title} is currently checked out and cannot be removed`
      );
      return;
    }
